...
public class ZoomView extends FrameLayout {
    ...
    public void smoothZoomTo(final float zoom, final float x, final float y) {
        ...
        Log.d("zoomView", "smoothZoomTo:"+zoom+":"+x+":"+y);
    }
...
//          getRootView().invalidate();
    ...
    private boolean onlyLongPressedZoom = true;
    ...
    private void processSingleTouchEvent(final MotionEvent ev) {
        ...
        if (!onlyLongPressedZoom && showMinimap && smoothZoom > 1.0f && touchingMiniMap) {
    ...
    private void processSingleTouchOutsideMinimap(final MotionEvent ev) {
        ...
        case MotionEvent.ACTION_DOWN:
            ...
            postDelayed(mPendingLongPressed, 1500);
            break;

        case MotionEvent.ACTION_MOVE:
            removeLongPressPender();
        ...
        case MotionEvent.ACTION_UP:
            removeLongPressPender();
            // tap
            if (onlyLongPressedZoom || l < 30.0f) {
                // checkLongPressEvent(ev, x, y);
                lastTapTime = System.currentTimeMillis();
    ...
    private CheckForLongPress mPendingLongPressed = new CheckForLongPress();
    private void removeLongPressPender(){
        if(mPendingLongPressed != null){
            removeCallbacks(mPendingLongPressed);
        }
    }
    private final class CheckForLongPress implements Runnable {
        ...
        public void run() {
            if ( !scrolling ) {
                checkLongPress(touchStartX, touchStartY);
                invalidate();
            }
        }
    }

    private void checkLongPress(float x, float y){
        if (smoothZoom == 1.0f) {
            smoothZoomTo(maxZoom, x, y);
        } else {
            smoothZoomTo(1.0f, getWidth() / 2.0f, getHeight() / 2.0f);
        }
        Log.d("zoomView", "on long press");
    }

    // check long pressed
    private boolean checkLongPressEvent(MotionEvent ev, float x, float y){
        // check double tap
